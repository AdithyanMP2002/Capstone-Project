...
import json
from src.backend import run_demo
from src.mock_data import mock_data
...
if "messages" in st.session_state:
    ...
    # Display The Chair's explanation explicitly OUTSIDE the expander
    if chair_message:
        st.markdown(f"""
        <div class="agent-card chair-card">
            <h3>⚖️ The Chair's Final Verdict</h3>
        </div>
        """, unsafe_allow_html=True)
        st.markdown(chair_message)
        st.markdown("---")

    # Collapse full transcript in an expander
    with st.expander("📋 View Full Debate Transcript (Other Agents)", expanded=False):
        st.caption(f"Question: *{st.session_state.question_asked}*")
        st.markdown("---")
        
        # Display each agent's response EXCEPT The Chair (since it's already shown)
        for msg in st.session_state.messages:
            agent = msg['agent']
            if agent == "TheChair":
                continue # Skip Chair, already shown above
